...
//		return;
//	}
//	Cola c = crearCola(pow2(altura(a)+1) - 1); // calculo lugar para todos los nodos del arbol como si fuera completo mas un nivel extra por los #
//	encolar(c, a);
//	while(!esVacia(c)){
...
char *abAStrAux(NodoAB *a) {
	...
	Cola c = crearCola(pow2(altura(a)+1) - 1); // calculo lugar para todos los nodos del arbol como si fuera completo mas un nivel extra por los #
	Cola salida = crearCola(pow2(altura(a)+1) - 1); // calculo lugar para todos los nodos del arbol como si fuera completo mas un nivel extra por los #
	encolar(c, a);
	while(!esVacia(c)){
		NodoAB* nodo = (NodoAB*)frente(c);
		if (nodo == NULL) {
